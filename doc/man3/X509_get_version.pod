...
certificate B<x>. These correspond to the constants B<X509V1_VERSION>,
B<X509V2_VERSION>, and B<X509V3_VERSION>. Note: the values of these constants
are defined by standards (X.509 et al) to be one less than the certificate
version. So B<X509V3_VERSION> has value 2 and b<X509V1_VERSION> has value 0.
...
number of certificate requests and CRLs. Note that, although they share version
constants, not all versions are defined for each type. RFC5280 defines X.509 v3
certificates but X.509 v2 CRLs. Only X.509 v1 certificate requests are defined.
